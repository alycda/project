...
Capture runs on a **Haiku Agent sub-agent**, not the parent's frontier model. The work is structured extraction — URL detection, normalization, classification — which Haiku handles at a fraction of the Opus/Sonnet cost with no measurable regression on this task. Frontier models stay reserved for Steps 0, 1, and 1.5.
...
Capture is the only cheap moment to record license status. The URL is already in
...
nothing and download is where the fetch happens anyway; a hundred extra HTTP requests
...
- `cited_in` — which workers cited this, e.g. `[theory, tooling]` or `[theory, claude-deep-research]`. Do not dedupe across workers: "everyone found it" and "only the deep pass found it" are different signals.
...
- `accessed` — ISO 8601 fetch date (download fills this)
- `archived` — snapshot URL, or `none  # save failed <date>, <reason>` (download fills this)
...
- **Repo URLs vs. file URLs.** `github.com/owner/repo` (clone the repo) vs. `github.com/owner/repo/blob/main/file.py` (just save the file). Classify correctly so download uses the right downloader.
...
- **Re-runs as more research streams in.** This is the core reason for upsert. Don't lose existing `status: done` / `path:` annotations on re-run.
- **Short URLs and redirects.** `bit.ly/...`, `tinyurl/...`, `t.co/...` should be expanded before storing. Some shorteners 404 over time.
- **Haiku misclassification.** Haiku is good at all four sub-tasks (detect / normalize / classify / flag) but a misclassified `github.com/owner/repo/blob/main/file.py` as `repo` causes download to `git clone` something that should be a single-file save. If you see any download download failures attributable to wrong `kind`, escalate that one entry's classification to the parent provider — don't escalate the whole step.
- **Hosted DR arXiv hallucination.** `*-deep-research.md` files occasionally cite plausible-looking arXiv IDs that don't resolve. Accept them as `kind: paper` and let the download phase fail loudly on the 404 — that is the natural check, and cheaper than verifying by reading. For arXiv IDs dated after the current month, add a `notes: "post-dated arxiv ID — verify before download"` flag so the user can spot-check.
